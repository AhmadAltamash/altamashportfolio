...
const ProfileImage = memo(({ avatarUrl }) => (
...
const StatCard = memo(({ icon: Icon, color, value, label, description, animation }) => (
  <div data-aos={animation} data-aos-duration={1300} className="relative group">
    <div className="relative z-10 bg-[var(--card-bg)] backdrop-blur-lg rounded-2xl p-6 border border-[var(--border-color)] overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-2xl h-full flex flex-col justify-between">
      <div className={`absolute -z-10 inset-0 bg-gradient-to-br ${color} opacity-10 group-hover:opacity-20 transition-opacity duration-300`}></div>
      
      <div className="flex items-center justify-between mb-4">
        <div className="w-16 h-16 rounded-full flex items-center justify-center bg-[var(--card-bg-hover)] transition-transform group-hover:rotate-6">
      ...
      <div>
        ...
        <div className="flex items-center justify-between">
          ...
          <ArrowUpRight className="w-4 h-4 text-text-primary/50 group-hover:text-[var(--text-primary)] transition-colors" />
        </div>
      </div>
    </div>
  </div>
...
const AboutPage = () => {
  // Seed from cache immediately (fast paint for repeat visitors), then
  // reconcile against a live Firestore count. The old version only ever
  // read localStorage once at mount — on a visitor's first-ever page
  // load, Portofolio.jsx's Firestore fetch hadn't written the cache yet,
  // so these counters permanently showed 0 until a reload.
  const [totalProjects, setTotalProjects] = useState(
    () => (JSON.parse(localStorage.getItem("projects") || "[]")).length
  );
  const [totalCertificates, setTotalCertificates] = useState(
    () => (JSON.parse(localStorage.getItem("certificates") || "[]")).length
  );
  ...
  const YearExperience = siteSettings?.yearsOfExperience ?? computedYears;

  useEffect(() => {
    (async () => {
      try {
        const snap = await getDoc(doc(db, "site-settings", "main"));
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          ...
          setTotalCertificates(certSnap.size);
        }
      } catch (err) {
    ...
    >
      ...
      <div className="w-full mx-auto pt-8 sm:pt-12 relative">
        <div className="flex flex-col-reverse lg:grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <ProfileImage avatarUrl={siteSettings?.avatarUrl} />
        </div>
