...
const Comment = memo(({ comment, formatDate }) => (
    <div 
        className="px-4 pt-4 pb-2 rounded-xl bg-[var(--card-bg)] border border-[var(--border-color)] hover:bg-[var(--card-bg-hover)] transition-all group hover:shadow-lg hover:-translate-y-0.5"
        ...
        <div className="flex items-start gap-3 ">
            ...
            <div className="flex-grow min-w-0">
                <div className="flex items-center justify-between gap-4 mb-2">
                    <h4 className="font-medium text-[var(--text-primary)] truncate">{comment.userName}</h4>
                    <span className="text-xs text-[var(--text-muted)] whitespace-nowrap">
                        {formatDate(comment.createdAt)}
                    </span>
                </div>
                <p className="text-[var(--text-secondary)] text-sm break-words leading-relaxed relative bottom-2">{comment.content}</p>
            </div>
        </div>
    </div>
));
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2" data-aos="fade-up" data-aos-duration="1000">
                ...
                <input
                    ...
                    className="w-full p-3 rounded-xl bg-[var(--card-bg)] border border-[var(--border-color)] text-[var(--text-primary)] placeholder-[var(--text-muted)] focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all"
                    required
                />
            </div>
            ...
            <div className="space-y-2" data-aos="fade-up" data-aos-duration="1200">
                ...
                <textarea
                    ...
                    className="w-full p-4 rounded-xl bg-[var(--card-bg)] border border-[var(--border-color)] text-[var(--text-primary)] placeholder-[var(--text-muted)] focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all resize-none min-h-[120px]"
                    required
                />
            </div>
            ...
            <div className="space-y-2" data-aos="fade-up" data-aos-duration="1400">
                <label className="block text-sm font-medium text-[var(--text-primary)]">
                    Profile Photo <span className="text-[var(--text-muted)]">(optional)</span>
                </label>
                <div className="flex items-center gap-4 p-4 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl">
                    {imagePreview ? (
...
const Komentar = () => {
    ...
    const uploadImage = useCallback(async (imageFile) => {
        ...
        const uploadPreset = import.meta.env.VITE_CLOUDINARY_UPLOAD_PRESET || "xpc8l9mz";
        const cloudName = import.meta.env.VITE_CLOUDINARY_CLOUD_NAME || "du7xgosqr";
    
        if (!uploadPreset) {
            console.error("Cloudinary upload preset variables are missing.");
            return null;
        }
        if(!cloudName) {
            console.error("Cloudinary cloudname variables are missing.");
        }
    
        const formData = new FormData();
        formData.append("file", imageFile);
        formData.append("upload_preset", uploadPreset); 
        formData.append("folder", "Portfolio/portfolio-comments-icon");
    ...
    return (
        ...
        <div className="p-6 border-b border-[var(--border-color)]" data-aos="fade-down" data-aos-duration="800">
